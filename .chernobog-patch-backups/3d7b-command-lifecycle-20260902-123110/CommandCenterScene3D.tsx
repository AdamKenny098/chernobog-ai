...
const CHERNOBOG_CORE_STATES: readonly ChernobogCoreState[] = [
  "idle",
  "listening",
  "thinking",
  "routing",
...
function isChernobogCoreState(value: unknown): value is ChernobogCoreState {
  return (
    typeof value === "string" &&
    CHERNOBOG_CORE_STATES.includes(value as ChernobogCoreState)
  );
}
...
function BespokeChernobogCore({
  ...
  const [requestedState, setRequestedState] =
    useState<ChernobogCoreState>("idle");
  ...
  useEffect(() => {
    function handleCoreState(event: Event) {
      const customEvent = event as CustomEvent<{ state?: unknown }>;
      const nextState = customEvent.detail?.state;

      if (isChernobogCoreState(nextState)) {
        setRequestedState(nextState);
      }
    }

    window.addEventListener("chernobog:core-state", handleCoreState);

    return () => {
      window.removeEventListener("chernobog:core-state", handleCoreState);
    };
  }, []);
  ...
  useEffect(() => {
    const actionEntries = Object.entries(actions).filter(
      (entry): entry is [string, THREE.AnimationAction] =>
        Boolean(entry[1]),
    );
    ...
    const keywords: Record<ChernobogCoreState, string[]> = {
      idle: ["idle"],
      listening: ["focus", "idle"],
      thinking: ["thinking"],
      routing: ["thinking", "focus"],
      speaking: ["speaking"],
      waiting: ["focus", "idle"],
      success: ["focus", "idle"],
      failure: ["thinking", "focus"],
    };

    const desiredKeywords = keywords[visualState];
    const matched = actionEntries.find(([name]) =>
      desiredKeywords.some((keyword) =>
        name.toLowerCase().includes(keyword),
      ),
    );

    const selected = matched?.[1] ?? actionEntries[0][1];
    ...
    const speedByState: Record<ChernobogCoreState, number> = {
      idle: 0.7,
      listening: 0.9,
      thinking: 1.45,
      routing: 1.75,
    ...
    selected
      ...
      .setEffectiveTimeScale(
        reduceMotion ? 0.01 : speedByState[visualState],
      )
      .fadeIn(0.22)
      .play();
  ...
  }, [actions, reduceMotion, visualState]);
  ...
  useFrame(({ clock }, delta) => {
    ...
    const scaleMultiplier: Record<ChernobogCoreState, number> = {
      idle: 1,
      listening: 1.025,
      thinking: 1.04,
      routing: 1.075,
    ...
    const rotationSpeed =
      visualState === "thinking" || visualState === "routing"
        ? 0.72
    ...
    root.current.rotation.y = THREE.MathUtils.damp(
      root.current.rotation.y,
      active && !reduceMotion
        ? Math.sin(clock.elapsedTime * rotationSpeed) *
            rotationAmplitude
        : active
          ? 0
          : -0.25,
