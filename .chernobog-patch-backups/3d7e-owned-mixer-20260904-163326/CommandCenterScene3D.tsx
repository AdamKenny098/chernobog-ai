...
const CHERNOBOG_CORE_MODEL_URL = "/models/chernobog/chernobog-core-3d7d.glb";
...
function BespokeChernobogCore({
  ...
  const fadeOutAllActions = useCallback(
    (except?: THREE.AnimationAction) => {
  ...
  const playPerformanceAction = useCallback(
    (
      ...
      fadeOutAllActions(action);

      action.enabled = true;
      action.reset();
      action.clampWhenFinished = mode === "once";
      action.setEffectiveTimeScale(reduceMotion ? 0.001 : 1);
    ...
    [actions, fadeOutAllActions, reduceMotion],
  );
  ...
  useEffect(() => {
    ...
    if (reduceMotion) {
      playPerformanceAction(nextLoopName, "loop", 0.08);
