...
function ProceduralCore({ isRootCore }: { isRootCore: boolean }) {
  ...
  useFrame(({ clock }, delta) => {
    const targetScale = isRootCore ? 1 : 0.38;
...
function HierarchyNode3D({
  ...
  const x = ((position.x - 50) / 100) * viewport.width;
  const y = ((48 - position.y) / 100) * viewport.height;
    ...
    if (group.current) {
      ...
      if (!reduceMotion) {
        group.current.position.y =
          y + Math.sin(clock.elapsedTime * 0.7 + index * 0.8) * 0.035;
      }
    }
  ...
  return (
    <group ref={group} position={[x, y, 0.28]}>
...
function OrbitalScaffold({
  ...
  return (
    <>
      ...
      <ProceduralCore isRootCore={current.kind === "core"} />
