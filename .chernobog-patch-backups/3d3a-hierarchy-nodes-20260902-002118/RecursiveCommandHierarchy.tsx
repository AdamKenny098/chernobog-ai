...
export function RecursiveCommandHierarchy() {
  ...
  useEffect(() => {
    window.dispatchEvent(
      new CustomEvent("chernobog:hierarchy-focus", {
        detail: {
          id: current.id,
          kind: current.kind,
        },
  ...
  return (
    <section className="relative z-10 min-h-[860px] overflow-hidden">
      ...
      >
        ...
        {children.length === 0 && !centerSelected ? (
          <div className="pointer-events-none absolute bottom-9 left-1/2 -translate-x-1/2 text-[8px] uppercase tracking-[0.22em] text-[#5f4128]">
            End of defined command layer ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â press Esc or Back to return
          </div>
        ) : null}
      </div>
    </section>
