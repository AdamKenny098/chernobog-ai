...
const MAX_DEPENDENCY_RELATIONSHIPS = 12;
const MAX_STRUCTURAL_RELATIONSHIPS = 6;
    ...
    if (relationships.length > 0) {
      ...
      if (dependencyRelationships.length > 0) {
        ...
        sections.push(
          "",
          "World Model explicit dependency relationships:",
          "- none currently represented in the selected canonical 11J evidence.",
        );
      }
      ...
      if (otherRelationships.length > 0) {
        sections.push(
          "",
          "World Model other relationships (state/structural evidence; current evidence first):",
