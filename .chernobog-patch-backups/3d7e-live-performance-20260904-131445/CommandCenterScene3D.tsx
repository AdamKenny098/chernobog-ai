...
function BespokeChernobogCore({
  ...
  useFrame(({ clock }, delta) => {
    ...
    root.current.scale.setScalar(nextScale);
    const depthByState: Record<ChernobogCoreState, number> = {
      idle: 0.15,
      listening: 0.48,
      thinking: 0.28,
      routing: 0.72,
    ...
    root.current.position.z = THREE.MathUtils.damp(
      root.current.position.z,
      active ? depthByState[visualState] : -3.8,
      damping,
      delta,
    );
    ...
    if (!active) {
      ...
      switch (visualState) {
        case "idle":
          targetYaw = Math.sin(t * 0.22) * 0.035;
          targetPitch = Math.sin(t * 0.17) * 0.012;
          break;

        case "listening":
          targetYaw = Math.sin(t * 0.48) * 0.015;
          targetPitch = -0.055;
          break;
        ...
        case "thinking":
          targetYaw =
            Math.sin(t * 1.7) * 0.11 +
            Math.sin(t * 0.43) * 0.035;
          targetRoll = Math.sin(t * 1.15) * 0.04;
          break;

        case "routing":
          targetYaw = Math.sin(t * 3.2) * 0.18;
          targetPitch = Math.sin(t * 2.4) * 0.065;
          targetRoll = Math.sin(t * 4.1) * 0.055;
          break;

        case "speaking":
          targetYaw = Math.sin(t * 0.42) * 0.055;
          break;
        ...
        case "success":
          targetYaw = Math.sin(t * 0.8) * 0.035;
          targetPitch = -0.08;
          break;

        case "failure":
          targetYaw =
            Math.sin(t * 8.5) * 0.045 +
            Math.sin(t * 2.1) * 0.018;
          targetPitch = 0.115;
          targetRoll = Math.sin(t * 10.5) * 0.025;
          break;
      }
    }
