/* eslint-disable @typescript-eslint/no-explicit-any -- partial runtime mocks in acceptance verifier */
...
async function main(): Promise<void> {
  ...
  /*
   * The v1 verifier scanned the whole helper module. That became stale when
   * explicit correction association was added to the same module. The
   * invariant belongs specifically to captureLiveLearningIngress().
   */
  ...
  for (
    ...
    assert.equal(
      ...
      false,
      `Live capture function must not perform unsafe or premature action: ${forbidden}`,
    );
  }

  console.log(
    "PASS live capture function remains experience-only even though the module now contains explicit correction association",
  );
