...
function formatCompactDependencyRelationship(
  entry: ScoredItem<WorldModelRelationship>,
): string {
  const relationship = entry.item;
...
function buildDependencyChains(
  ...
  const requiresModel = relationships.filter(
    (entry) =>
      entry.item.type === "requires-model",
  );
  ...
  const servedBy = new Map(
    relationships
      .filter(
        (entry) =>
          entry.item.type === "served-by",
      )
      .map((entry) => [
        entry.item.fromEntityId,
        entry,
      ]),
  );

  return requiresModel.map((required) => {
    const provider =
      servedBy.get(required.item.toEntityId);

    const base =
      `${required.item.fromEntityId} ` +
      `--requires-model--> ` +
      `${required.item.toEntityId}`;

    if (!provider) {
      return (
        `DEPENDENCY_CHAIN ${base} ` +
        `[provider-edge=missing]`
      );
    }
    ...
    return (
      `DEPENDENCY_CHAIN ${base} ` +
      `--served-by--> ` +
      `${provider.item.toEntityId} ` +
      `[requiresEvidence=${evidenceText(required.item.evidence)}; ` +
      `servedByEvidence=${evidenceText(provider.item.evidence)}]`
    );
  });
}
...
export async function buildChernobogWorldModelContext():
  ...
  try {
    ...
    const sections: string[] = [
      ...
      `- RELATIONAL_STATUS: ${explicitDependencyRelationships.length > 0 ? "substantive" : "none"}`,
      `- explicit dependency relationships exposed: ${explicitDependencyRelationships.length}`,
      `- complete dependency chains exposed: ${dependencyChains.length}`,
      ...
      `- SUPPORTED_PREDICTION_STATUS: ${supportedPredictions.length > 0 ? "supported" : "none"}`,
    ...
    sections.push(
      ...
      `RELATIONAL_STATUS=${explicitDependencyRelationships.length > 0 ? "substantive" : "none"}`,
      `SUPPORTED_PREDICTION_STATUS=${supportedPredictions.length > 0 ? "supported" : "none"}`,
      `DEPENDENCY_EDGE_COUNT=${explicitDependencyRelationships.length}`,
      `DEPENDENCY_CHAIN_COUNT=${dependencyChains.length}`,
      ...explicitDependencyRelationships.map(
        formatCompactDependencyRelationship,
      ),
      ...dependencyChains,
    );
    ...
    sections.push(
      ...
      supportedPredictions.length > 0
        ? `- supported predictions: ${supportedPredictions.length}`
        : "- No supported predictions.",
      `- stored predictions: ${snapshot.predictions.length}`,
      `- insufficient predictions: ${insufficientPredictionCount}`,
    );
