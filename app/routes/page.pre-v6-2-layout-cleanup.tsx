...
function RouteCard({ route }: { route: RegistryRoute }) {
  ...
  return (
    <article className="relative overflow-hidden border border-[#6f3d18]/70 bg-[#090705]/95 p-4 shadow-[inset_0_0_0_1px_rgba(255,154,58,0.07)] transition hover:border-[#c9782f]/80">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
        <div>
          ...
          <div className="mt-2 font-mono text-xs text-[#c9782f]/80">
            {route.path}
          </div>
        </div>
        ...
        <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_280px]">
          <main className="grid gap-4">
            <header className="border border-[#6f3d18]/70 bg-[#080604]/95 p-4 shadow-[inset_0_0_0_1px_rgba(255,154,58,0.08)]">
              <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                <div>
                  ...
                  <p className="mt-3 max-w-4xl text-sm leading-6 text-[#b7a386]">
                    Registry-backed route telemetry for command pages, vault lanes, review workspaces, module surfaces, Internal APIs, debug channels, and sealed legacy paths.
                  </p>
                </div>
                <div className="flex flex-wrap gap-2">
                  <Link
                    href="/command-center"
                    className="border border-[#c9782f]/70 bg-[#2a1609]/70 px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.22em] text-[#ffd28a] hover:bg-[#3a1e0c]"
                  >
                    Back to Command Center
                  </Link>
                  <Link
                    href="/command"
                    className="border border-[#6f3d18]/70 bg-black/30 px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.22em] text-[#b7a386] hover:text-[#ffe0ac]"
                  >
                    Console
                  </Link>
                  <Link
            ...
            <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-5">
              <ClassicStat label="Registry Entries" value={allRoutes.length} detail="known route signatures" />
              <ClassicStat label="Primary Navigation" value={primaryRoutes.length} detail="operator nav" />
              <ClassicStat label="Active Routes" value={activeCount} detail="live lanes" />
              <ClassicStat label="Experimental" value={experimentalCount} detail="unstable lanes" />
              <ClassicStat label="Internal / Sealed" value={internalCount} detail="api or hidden" />
            </section>

            <ClassicPanel eyebrow="Filter Matrix" title="Route Scan Controls">
              <form action="/routes" className="grid gap-3 xl:grid-cols-[1fr_150px_150px_170px_auto]">
                ...
                <div className="flex items-end gap-2">
                  ...
                  <Link
                    href="/routes"
                    className="flex h-11 items-center border border-[#6f3d18]/70 bg-black/35 px-4 text-[10px] font-semibold uppercase tracking-[0.22em] text-[#8e785c] hover:text-[#ffe0ac]"
                  >
                    Reset
                  </Link>
                </div>
            ...
            <ClassicPanel eyebrow="Distribution" title="Kind Distribution">
              <div className="grid gap-2 md:grid-cols-3 2xl:grid-cols-9">
                {kindCounts.map((item) => (
                  ...
                  >
                    ...
                    <div className="mt-2 font-mono text-lg text-[#ffd28a]">
                      {item.count}
                    </div>
                  </Link>
                ))}
          ...
          <aside className="grid content-start gap-4">
            <ClassicPanel eyebrow="Registry" title="Route Registry Health">
            ...
            <ClassicPanel eyebrow="Seal Logic" title="Open Link Protection">
